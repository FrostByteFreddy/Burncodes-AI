...
def process_single_url_task(self, task_id: int, tenant_id: UUID, parent_url: str = None):
    ...
    try:
        ...
        if crawl_result and crawl_result.success and crawl_result.markdown:
            ...
            # Link discovery: use soup extractor on the rendered HTML for maximum coverage
            # crawl4ai's built-in link list often misses anchors; BeautifulSoup parses the
            # fully-rendered DOM that Playwright already fetched.
            rendered_html = getattr(crawl_result, "html", None) or ""
            if rendered_html:
                soup_links = extract_internal_links(rendered_html, url)
                ...
                # Fallback to crawl4ai's own link list if HTML wasn't captured
                for link in crawl_result.links.get("internal", []):
                    ...
                    if not is_excluded_link:
                        found_links.add(href)
            error_logger.info("playwright: found %d links on %s", len(found_links), url)
