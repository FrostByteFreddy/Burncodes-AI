  ...
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center">
    <div class="bg-base-100 p-8 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          <label for="rag_prompt_template" class="block text-sm font-medium text-base-content">RAG Prompt Template</label>
          <textarea v-model="formData.rag_prompt_template" id="rag_prompt_template" rows="8"
            class="w-full p-2 mt-1 bg-base-200 border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"></textarea>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label for="doc_language" class="block text-sm font-medium text-base-content">Document Language</label>
              <input v-model="formData.doc_language" type="text" id="doc_language" placeholder="e.g., en"
                class="w-full p-2 mt-1 bg-base-200 border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            <div>
              <label for="translation_target" class="block text-sm font-medium text-base-content">Translation Target</label>
              <input v-model="formData.translation_target" type="text" id="translation_target" placeholder="e.g., English"
                class="w-full p-2 mt-1 bg-base-200 border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
        </div>
        <div class="d-none">
            <label for="doc_description" class="block text-sm font-medium text-base-content">Document Description</label>
            <input v-model="formData.doc_description" type="text" id="doc_description"
            class="w-full p-2 mt-1 bg-base-200 border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
        </div>
         <div class="d-none">
            <label for="source_description" class="block text-sm font-medium text-base-content">Source Description</label>
            <input v-model="formData.source_description" type="text" id="source_description"
            class="w-full p-2 mt-1 bg-base-200 border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
        </div>
        <div class="d-none">
            <label for="last_updated_description" class="block text-sm font-medium text-base-content">Last Updated Description</label>
